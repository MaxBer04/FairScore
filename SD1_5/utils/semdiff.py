...
class Diffusion:
    ...
    def sample_variance_noise(self, num_samples = 1, seed=None):
        ...
        """
        if seed is None: seed = torch.randint(int(1e6), (1,))
        return torch.randn( (num_samples,)+self.variance_noise_shape,
                            generator=torch.manual_seed(seed)
                            ).to(self.device)
...
class StableDiffusion(Diffusion):
    ...
    def get_h_shape(self):
        """
        Return the shape for the h tensors
        """
        xT = self.unet.sample()
        uncond_embedding = self.encode_text("")
        with torch.no_grad():   
            out = self.unet.forward(xT, timestep = self.timesteps[-1], encoder_hidden_states = uncond_embedding)
        return (self.num_inference_steps,) + tuple(out.h.shape[1:])
    ...
    def forward(self, x0, 
        ...
        uncond_embedding = self.encode_text([""]*x0.shape[0])
        for t in op:
...
class Q:
    ...
    """
    def __init__(self, x0 = None, xT = None, w0 = None, wT = None, hs = None, zs = None, etas = None, delta_hs = None, seed = None, asyrp = None, prompts = [""], cfg_scale = None, label = None):    
...
class SemanticDiffusion(Interpolations):
    ...
    def sample(self, decode = True, 
        ...
        """
        if seed is None: seed = torch.randint(int(1e6), (1,))
        q = Q(seed = seed, etas = etas, prompts = prompts)
...
class StableSemanticDiffusion(SemanticDiffusion):
    ...
    def sample(self, decode = True, 
        ...
        if seed is None: seed = torch.randint(int(1e6), (1,))
        q = Q(seed = seed, etas = etas, prompts = prompts)
        samples = self.diff.unet.sample(num_samples=len(prompts), seed = seed)
        q.wT = samples
        if not etas is None: 
