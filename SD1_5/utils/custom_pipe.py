...
class HUNet2DConditionModel(UNet2DConditionModel):
    ...
    def forward(
        ...
        # 2. pre-process
        print('#'*40)    
        print(f"Before conv_in size: {sample.size()}")
        ...
        print(f"Input size: {sample.size()}")
        ...
        print(f"h!!!! sample size: {sample.size()}")
...
class HDiffusionPipeline(StableDiffusionPipeline):
        ...
        **kwargs,
        ...
        def cond_fn(latents, t):
            with torch.enable_grad():
                ...
                return torch.autograd.grad(selected.sum(), latents_in)[0] * classifier_scale
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_pred_grad = cond_fn(latents, t)
                betas = betas_for_alpha_bar(num_inference_steps)
                
                noise_pred = noise_pred + betas[i] * noise_pred_grad
...
class CustomDiffusionPipeline(StableDiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                if i <= num_inference_steps - (num_inference_steps*t_frac):
                  continue
                if self.interrupt:
                    continue
